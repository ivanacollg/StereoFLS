...
class stereoSonar:
    ...
    def callback(self, msgVertical, msgHorizontal):
        ...
        imgHorizontal = 255.*np.array(self.BridgeInstance.imgmsg_to_cv2(msgHorizontal)).astype(float)
        imgHorizontal = np.array(imgHorizontal).astype(int)
        ...
        imgVertical = 255.*np.array(self.BridgeInstance.imgmsg_to_cv2(msgVertical)).astype(float)
        imgVertical = np.array(imgVertical).astype(int)
        ...
        if match_status:
            ...
            header = Header()
            header.frame_id = "rexrov/forward_sonarvert_link"
            header.stamp = (
