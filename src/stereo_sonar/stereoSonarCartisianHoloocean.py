...
import holoocean
#from holoocean_wrapper import utils
...
class stereoSonar:
    ...
    def __init__(self, ns="~"):
        ...
        self.imageHorizontalPub = rospy.Publisher("hori_features",Image,queue_size = 5)
        self.imageVerticalPub = rospy.Publisher("vert_features",Image,queue_size = 5)
    ...
    def generate_holoocean_map(self, config: dict) -> np.array:
        ...
        _res = (config['RangeMax'] - config['RangeMin']) / 512
        _height = _res * config['RangeBins']
        _rows = config['RangeBins']
        _width = np.sin(np.radians(horizontal_fov / 2)) * _height * 2
    ...
    def callback(self, msgVertical, msgHorizontal):
        ...
        self.generate_holoocean_map(self.holoocean_config)
        ...
        imgVertical = cv2.fastNlMeansDenoising(imgVertical, None, 15, 7, 21)
        imgHorizontal = cv2.fastNlMeansDenoising(imgHorizontal, None, 15, 7, 21)
        ...
        # Visualize FOV
        cv2.line(imgHorizontal,(334,0),(334,1000),[0,0,255],5)
        cv2.line(imgHorizontal,(177,0),(177,1000),[0,255,0],5)
        cv2.line(imgVertical,(334,0),(334,1000),[0,0,255],5)
        cv2.line(imgVertical,(177,0),(177,1000),[0,255,0],5)
        ...
        #self.imgPub.publish()
        # Visualize features 
        imgHorizontal1 = imgHorizontal.copy()
        imgVertical1 = imgVertical.copy()
        for loc in horizontalFeatures:
                cv2.circle(imgHorizontal1, (loc[1],loc[0]),5, (255), -1)
        for loc in verticalFeatures:
                cv2.circle(imgVertical1, (loc[1],loc[0]),5, (255), -1)
        vis_img = cv2.normalize(
            imgVertical1, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        vis_img = cv2.applyColorMap(imgVertical1, 2)
        img_msg = ros_numpy.image.numpy_to_image(vis_img, "bgr8")
        img_msg.header.stamp =  msgHorizontal.header.stamp
        self.imageVerticalPub.publish(img_msg)
        vis_img = cv2.normalize(
            imgHorizontal1, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
        vis_img = cv2.applyColorMap(vis_img, 2)
        img_msg = ros_numpy.image.numpy_to_image(vis_img, "bgr8")
        img_msg.header.stamp =  msgHorizontal.header.stamp
        self.imageHorizontalPub.publish(img_msg)
